...
// Global queue for miner output (shared with KhutiWallet)
...
std::string KhutiWebServer::get_embedded_html() {
    ...
    if (!file.is_open()) {
        return "<h1>❤️ SarigR Rig</h1><p>index.html not found. Place it in the same folder as the binary.</p>";
    }
...
void KhutiWebServer::handle_client(int client_fd) {
    ...
    if (path == "/" || path == "/index.html") {
        ...
        response_body = "<h1>❤️ SarigR Rig</h1><p>Endpoint not found</p>";
    }
...
void KhutiWebServer::start(int port) {
    ...
    std::cout << "❤️ SarigR Web Dashboard: http://localhost:" << port << "\n";
